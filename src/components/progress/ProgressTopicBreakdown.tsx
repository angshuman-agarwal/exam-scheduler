        ...
        <table data-testid="progress-topic-table" className="min-w-full border-collapse">
          <thead>
            <tr className="border-b border-black/[0.06] text-left">
              ...
              <th className="px-5 py-3 text-[11px] font-semibold uppercase tracking-[0.04em] text-gray-400">Confidence</th>
              <th className="px-5 py-3 text-[11px] font-semibold uppercase tracking-[0.04em] text-gray-400">Recency</th>
              <th className="px-5 py-3 text-[11px] font-semibold uppercase tracking-[0.04em] text-gray-400">Status</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((row) => (
              <tr key={row.topic.id} data-testid="progress-topic-row" className="border-t border-black/[0.06] align-top hover:bg-black/[0.01] transition-colors">
                ...
                <td className="px-5 py-3.5">
                  <StatusCell row={row} onPlanNow={onPlanNow} />
                </td>
              </tr>
            ))}
          </tbody>
          ...
          {rows.map((row) => (
            <article key={row.topic.id} data-testid="progress-topic-row-mobile" className="px-4 py-3.5">
              ...
              <div className="mt-2.5 grid grid-cols-2 gap-x-4 gap-y-2.5">
                <div>
                  <p className="text-[10px] font-semibold uppercase tracking-[0.08em] text-gray-400">Confidence</p>
                ...
                <div>
                  <p className="text-[10px] font-semibold uppercase tracking-[0.08em] text-gray-400">Recency</p>
                  <p className="mt-1.5 text-[13px] text-gray-500">{row.recencyLabel}</p>
                </div>
              </div>
            </article>
          ))}
